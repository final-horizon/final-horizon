...
public sealed class CombatModeIndicatorsOverlay : Overlay
{
    ...
    {
        var viewportOffset = (args.ViewportControl as Control)?.GlobalPixelPosition ?? new Vector2i(0, 0);
        ...
        var mousePosFixed = mousePos - viewportOffset;
        ...
        if (sight != null)
        {
            ...
            if (rsiState.IsAnimated && rsiState.AnimationFrameCount >= 3)
            {
                ...
                DrawSightPartial(sightTexture, bracket1, bracket2, args.ScreenHandle, rot, mousePosFixed, scale, _main ?? sight.MainColor, _second ?? sight.StrokeColor, offset, viewportOffset, bracket3, bracket4);
            }
            else
            {
                var sightTexture = spriteSys.Frame0(sight.Sprite);
                DrawOverlayPart(sightTexture, args.ScreenHandle, mousePosFixed, scale, _main ?? sight.MainColor, _second ?? sight.StrokeColor);
            }
        }
    ...
    private void DrawSightPartial(Texture sight, Texture bracket1, Texture bracket2, DrawingHandleScreen screen, float rotation, Vector2 centerPos, float scale, Color mainColor, Color strokeColor, float offset, Vector2 viewportOffset, Texture? bracket3 = null, Texture? bracket4 = null)
    {
        ...
        if (_rotation)
        {
            screen.SetTransform(Matrix3x2.CreateRotation(rotation, centerPos + viewportOffset));
            centerPos += viewportOffset;
        }
